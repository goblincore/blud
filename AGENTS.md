...
Implementation runs via `~/go/bin/dispatch-ui` at <http://localhost:8090> using model `zai/glm-5.1` on the `pi` harness. M1's 11 tasks are already queued at `~/.Codex/dispatch/plans/2026-04-20-blud-m1-task-{1..11}.md` with serial `depends_on` chain — trigger task-1 manually, the rest auto-flow.
