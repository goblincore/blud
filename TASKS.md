...
- `F2.dynamite-airburst` [ ] BUG (2026-06-17 playtest): a mid-air dynamite detonation (hits a zombie's body) shows the ground-style explosion and reads as "floating". ROOT CAUSE (source-confirmed; impact-detonate is CORRECT/default — not a fuse issue): NotBlood `actExplodeSprite` (actor.cpp:5995) picks the explosion ANIMATION by floor contact — `kThingArmedTNTBundle`: `florhit==0 → seqSpawn(4,…)` (AIR burst, SEQ 4) else `seqSpawn(3,…)` (GROUND, SEQ 3). There are ≥2 explosion variants; the AIR one is designed to look right mid-air. Blud uses ONE `explosion-placeholder` atlas (`src/vfx/explosion.ts`) regardless. FIX next session: extract NotBlood explosion SEQ 3 (ground) + SEQ 4 (air) [+ maybe napalm/small variants], and have `GibSystem.spawnExplosion` (`src/game/gibs/index.ts`) select air-vs-ground by detonation height and play the matching one.
