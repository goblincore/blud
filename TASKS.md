...
**BONE TUBES — SPEC APPROVED (2026-09-02), plan pending.** Skeleton out of the marched field: posed bone prims (unchanged upstream) drawn as ONE instanced analytic tube mesh in the polygonal pass; the march composite's depth test hides bone under flesh and reveals it in cavities. Organs stay in the field. Deletes the wound-zone bone fold (1.1–1.8 M capsule evals/frame measured) instead of culling it (gore r3 item 2). Gates: owner reel (torso crater / head shot / arm chunk; bone through skin = fail) + boneEvals counter → 0 for bones. Branch `claude/bone-tubes`; execute via dispatch-ui.
[spec](docs/superpowers/specs/2026-09-02-bone-tubes-design.md)
