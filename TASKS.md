...
- [~] 4c **Night Train look v2** (owner, after playing): a grimy industrial prison train. Part 1 restyle [spec](docs/superpowers/specs/2026-09-25-night-train-art-v2-design.md) · [plan](docs/superpowers/plans/2026-09-25-night-train-art-v2.md) **built** (kit v2: 54 pieces, 11 materials baked to colour/roughness/normal, art 7.4 MB; 202 instanced meshes). Walk, windows, sway pass; budget raised to +350 draws / +12 ms (owner, 2026-09-26; optimise later: small-dressing shadows off, then texture atlas) → part 2 flashlight pickup + dynamic lights ([spec](docs/superpowers/specs/2026-09-26-night-train-dynamic-light-design.md): shadowed window light for lightning + sweeps, lamp moods + scripted blackouts, furnaces; **waits on the game loop**) → part 3 haze + raymarched volumetric light for every light.
- [ ] 4f **Game loop first** (owner, 2026-09-26): [Wake Plan 2](docs/superpowers/plans/2026-09-11-wake-2-game-loop.md) refreshed with Night Train as its test level — health, damage, death, pickups, loadout, triggers/events, gates, completion. Unblocks the flashlight pickup, light events, keys, encounters.
- [ ] 4e Gameplay: encounters (wake-up triggers), the Stoker; dawn later.
- [ ] 4d **Owner ideas (2026-09-25), for layout draft 2:** keys + locked doors to gate progress (retro FPS staple; **coloured keys as placeholders** — red/blue/yellow, re-skinnable later; format has gates, needs key pickups, locked-door feedback, a key HUD); the train a bit longer (bring back third class / coat check / tender, or lengthen carriages).
