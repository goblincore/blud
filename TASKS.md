...
- `P7`  [ ]  **Extract BUNFUSE (QAV 21) + use during cooking** — our current `dynamite-idle` is Blood's BUNIDLE (6-frame calm loop); the authentic "fuse burning down" visual is BUNFUSE (QAV 21, 66 frames ~5.5s at tpf=10, with spark-tile cycling 3195/3196/3197 and a `callbackId=2` at frame 65 for self-explode). `scripts/extract_qav.py:27` needs `21: "dynamite-fuse-burn"` added + re-run. Then in [dynamite.ts](src/game/weapons/dynamite.ts) swap the `cooking` state to `fpAnimator.restart('dynamite-fuse-burn', ...)`. Nice-to-have polish; fix landed in `ce075d0`-follow-up made BUNIDLE the cooking visual which is visually calm but not authentic. See [docs/dev-notes/2026-04-22-notblood-source-reference.md](docs/dev-notes/2026-04-22-notblood-source-reference.md) + Obsidian reference for full BUNFUSE frame dump.
