...
- `X1.gib-freeze` [ ] **Persistent WebGPU chunk-view pool** — X1.27 dependency is clear; reviewed implementation draft is at [2026-08-17-sdf-gib-freeze.md](docs/superpowers/plans/2026-08-17-sdf-gib-freeze.md). Prove whole-view reuse first; accept only visible-window first/repeated 18-piece gibs with no gameplay frame over 50 ms.
...
- `X1.hand-followups` [-] **FPV hand polish** — make X1.27 clip+dynamite the initial FPV field, then extend the 35 mm wrist stub into a forearm that exits the bottom of frame. Preserve `prims|baked` debug modes and the baked→prims load fallback.
