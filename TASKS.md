...
- `F2.dynamite-airburst` [ ] BUG (2026-06-17 playtest): thrown dynamite detonates mid-air at the impact point on a zombie's body (floating explosion), not on the ground. ROOT-CAUSE LEAD: NotBlood `ThrowBundle` (weapon.cpp:1220) uses Impact=1 only when `fuseTime<0`; a cooked bundle gets a TIMED fuse (`evPost`) → bounces off enemies/geometry, settles on the FLOOR, then explodes on the ground. Blud defaults `impactMode:true` (detonate on first collider contact, `dynamite.ts:140`). Next session: port the timed-fuse + bounce-to-floor behavior (faithful), OR cheaper, keep impact but spawn the explosion at ground level. User to verify NotBlood feel.
