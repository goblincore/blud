...
OUT = argv[0]; VAR = argv[1] if len(argv) > 1 else 'v4'
...
for name,(az,el) in {'left':(90,3),'threequarter':(50,24),'rear34':(132,20)}.items():
    ...
    sc.render.filepath=os.path.join(OUT,f'{VAR}-{name}.png')
...
sc.render.filepath=os.path.join(OUT,f'{VAR}-detail.png')
bpy.ops.render.render(write_still=True)
cam.data.lens = 50.0
...
sc.render.filepath=os.path.join(OUT,f'{VAR}-fpv.png')
bpy.ops.render.render(write_still=True)
