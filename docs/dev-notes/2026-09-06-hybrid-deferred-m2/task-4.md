...
- ~~The flashlight contribution multiplies existing ambient/emission/practical lighting without a dedicated flashlight-isolation frame in this run — accepted because the mask no longer fragments (the coordinator's isolation instruction was conditional on continued fragmentation) and the clear-flesh ratio 0.991 plus the unchanged practical glow in on/off captures shows other terms are not shadowed.~~ **CORRECTED 2026-09-06 (review fix): that statement was wrong about the source.** `deferredLight` gates the visibility multiplication on `shadowEnabled > 0.5 && f32(i) == shadowLightIndex` — it applies ONLY to the designated flashlight's direct contribution inside the light loop. Ambient is added before the loop, emission returns before it, and every non-designated light is untouched by construction, so there was never a term-mixing risk. What the original sentence groped at is real but different: the mask statistics cannot PER-PIXEL attribute a darkening to the flashlight term alone (a darkened pixel could in principle have dimmed for another reason), which is why the review-fix binding-identity gate now proves term isolation directly with constant-depth maps and raw linear per-pixel deltas (see shadow-binding-review-fix.md).
