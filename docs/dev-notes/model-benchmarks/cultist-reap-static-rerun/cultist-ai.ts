...
export class CultistBrain {
  ...
  // Tuning (read from CULTIST_TOMMY at construction).
  private readonly detectionRangeM: number;
  private readonly preferredRangeM: number;
  private readonly shotsPerBurst: number;
  private readonly shotIntervalMs: number;
  private readonly reloadSec: number;
  private readonly staggerSec: number;
  private readonly aimWindupSec: number;
  ...
  constructor(
    ...
    this.detectionRangeM = CULTIST_TOMMY.detectionRangeM;
    this.preferredRangeM = CULTIST_TOMMY.preferredRangeM;
    this.shotsPerBurst = CULTIST_TOMMY.shotsPerBurst;
    this.shotIntervalMs = CULTIST_TOMMY.shotIntervalMs / 1000; // ms → sec
    this.reloadSec = CULTIST_TOMMY.reloadSec;
    this.staggerSec = CULTIST_TOMMY.staggerSec;
    this.aimWindupSec = CULTIST_TOMMY.aimWindupSec;
  }
  ...
  update(dt: number, _self: Vec3, player: Vec3): void {
    ...
    if (this.state === CultistState.Shoot) {
      ...
      if (this.shotCooldown <= 0 && this.shotIndex < this.shotsPerBurst) {
        // Fire one shot
        this.shotIndex++;
        this.shotCooldown = this.shotIntervalMs;
        this.prevShot = true;
        this.hooks?.onShot?.();
      }
  ...
  /**
   * Returns a velocity vector pointing toward the player while chasing.
   * Returns zero vector when dead, aiming, shooting, reloading, or staggered.
   */
  desiredVelocity(self: Vec3, player: Vec3): Vec3 {
    if (this.state === CultistState.Dead || this.state === CultistState.Stagger) return { x: 0, y: 0, z: 0 };
    ...
    if (len > this.detectionRangeM) return { x: 0, y: 0, z: 0 };
