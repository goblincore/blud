...
A mesh bake extracts authored bone surfaces in rigid segment-local coordinates and poses them using existing rig transforms. It must preserve skull cavities, pelvis silhouette and ribs in the sense of recognizable, readable anatomy at broadly compatible scale and rigging — deliberate anatomy improvements (fewer ribs, a better mesh skull shape, corrected bend behaviour) are explicitly allowed per the 2026-09-08 owner clarification, provided they are labelled as intentional art differences, not smuggled in as parity. Tubes are not the geometry source. Mesh depth alone is insufficient: the current field's bone/flesh union, wound reveal and shading semantics must be reproduced or the mismatch documented as a failed approach. Share the forward authored lighting rules; do not switch to generic PBR or accept a second unmatched lighting model. Old world-axis bend/squash quirks are reference quirks, not requirements: intentional improved rigid bend behaviour is allowed and recorded, never reverted merely to imitate the old field.
...
Visual fidelity is a prerequisite for recommendation: matched camera, posed animation, wound state, light state, timestep and resolution; render repeatability floor measured first. The hard gates are correctness, not old-geometry parity: no clipping through intact flesh, no bone leaks, no broken wound reveals or severing, no distracting shading mismatches against the authored forward lighting. Compare skull/pelvis/ribs, intact flesh coverage, shallow/deep wounds, bent joints, severed segments, moving flashlight, darkness, and wet highlights; label intentional anatomy differences separately from defects. Capture and inspect normal/depth/material evidence as well as lit images. Never compensate with a global brightness change. A candidate can be rejected with useful evidence; do not relax criteria merely to finish a task.
