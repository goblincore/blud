...
const evaluate = async (expression) => { const r = await send('Runtime.evaluate', { expression, awaitPromise: true, returnByValue: true, timeout: 120_000 }); if (r.result?.exceptionDetails) throw new Error(JSON.stringify(r.result.exceptionDetails).slice(0, 400)); return r.result?.result?.value; };
...
await evaluate(`(async () => { const g = window.__sdfGame; g.woundPanel?.(false); g.gooPanel?.(false); g.setLoopRunning(false); g.teleport(1); g.step(2); g.freeze(true); function __hitWindow() { g.step(1); } window.__hitWindow = __hitWindow; window.__aimAndFire = async () => { g.setCrowd(null); g.freeze(true); for (let i = 0; i < 6; i++) g.step(1); await g.resolveGpu(); const z = g.zombies().find(z => z.room === 1); const zb = g.zombie(z.id); for (const dz of [1.2, -1.2, 1.6, -1.6]) for (const yaw of [0, Math.PI]) for (const pitch of [-0.35, -0.2, 0, -0.5]) { g.setPose(z.pos[0], z.pos[2] + dz, yaw, pitch); for (let i = 0; i < 3; i++) g.step(1); await g.resolveGpu(); let n = 0; while (!g.fire(2) && n < 600) { g.step(1); if (++n % 15 === 0) await g.resolveGpu(); } await g.resolveGpu(); const before = zb.woundCount(); const t0 = performance.now(); window.__hitWindow(); const ms = +(performance.now() - t0).toFixed(1); const d = zb.woundCount() - before; for (let i = 0; i < 3; i++) g.step(1); await g.resolveGpu(); if (d > 0) return { ms, wounds: d, dz, yaw, pitch }; } return null; }; const warm = await window.__aimAndFire(); return warm; })()`).then(w => console.log('warm shot:', JSON.stringify(w)));
await send('Profiler.enable');
await send('Profiler.setSamplingInterval', { interval: 100 });
await send('Profiler.start');
const timing = await evaluate(`(async () => { const out = []; for (let s = 0; s < 4; s++) out.push(await window.__aimAndFire()); return out; })()`);
