...
set -euo pipefail
cd "$(dirname "$0")/.."
OUT=${1:?outDir}; CELL=${2:-0.02}; BAND=${3:-0.02}; STEPS=${4:-8}
...
node scripts/perf-r2-parity.mjs capture "$OUT/1-wounded" "${COMMON[@]}" \
  --pre "__hullSpike.pelletAt('torso'); __hullSpike.slugAt('torso')"
...
node scripts/perf-r2-parity.mjs capture "$OUT/3-gib" "${COMMON[@]}" \
  --pre "__hullSpike.severLimb('armL'); __hullSpike.severLimb('legR'); await new Promise(r=>setTimeout(r, 400))"
