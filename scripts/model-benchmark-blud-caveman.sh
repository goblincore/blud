#!/bin/bash
# Caveman variant of model-benchmark-blud-cot.sh: same 2 tasks, but each
# prompt is prefixed with a terseness directive. Tests whether telling
# the model to be brief in its inter-tool commentary saves wall time on
# top of FSM-thinking compression.
#
# Usage: ./model-benchmark-blud-caveman.sh <run-name> <output-dir>
#
# Assumes llama-serve-lil running on :8888 with the desired think + grammar
# config (typically FSM-think defaults). Worktree at /tmp/blud-lil-bench.

set -euo pipefail

RUN_NAME="${1:?Usage: model-benchmark-blud-caveman.sh <run-name> <output-dir>}"
...
# Caveman prefix. Tries to suppress: "I'll do X", "Let me do Y", "Now I'll
# Z", post-action recaps ("Done! I added..."), and lengthy tool-call
# narration. Inspired by JuliusBrussee/caveman skill.
CAVEMAN_PREFIX='BRIEF MODE: Skip narration. No "I will", "Let me", "Now". No recap of what you did. Tool calls and final result only.
...
run_task() {
  ...
  START=$(date +%s)
  timeout 1500 lil --cwd "$WT" -p "${CAVEMAN_PREFIX}${prompt}" \
  ...
  echo "done (${ELAPSED}s, diff=${diff_lines}L, stdout=${stdout_bytes}B)"
...
EOF
      ...
      if ! npx tsx ./qc-probe.mts > /tmp/qc-probe.log 2>&1; then
        task_pass=false; task_notes="numeric values wrong"
      fi
      rm -f ./qc-probe.mts
    fi
  fi'
