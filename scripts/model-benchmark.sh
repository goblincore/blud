...
run_task() {
  ...
  local payload
  # chat_template_kwargs.enable_thinking=false is the hard way to force
  # non-thinking mode. Some GGUFs ignore the /no_think soft prefix (e.g.
  # majentik/Qwen3.6-35B-A3B-RotorQuant-GGUF-Q3_K_M) and keep emitting
  # <think> traces until max_tokens, never producing an answer. With
  # enable_thinking=false the Jinja template skips the thinking prompt.
  # preserve_thinking=true stops the template from emitting empty
  # <think></think> blocks for historical turns, which otherwise cause
  # KV-cache drift (see QwenLM/Qwen3.6#131, badlogic/pi-mono#3325).
  payload=$(jq -n --arg sys "You are a coding assistant. Be concise, produce clean working TypeScript. No explanations unless asked." --arg prompt "/no_think
$prompt" '{model:"default",messages:[{role:"system",content:$sys},{role:"user",content:$prompt}],max_tokens:4000,temperature:0.7,top_p:0.8,top_k:20,stream:false,chat_template_kwargs:{enable_thinking:false,preserve_thinking:true}}')
