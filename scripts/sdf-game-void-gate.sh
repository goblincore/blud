#!/usr/bin/env bash
# Headless gate for the Void (new game start). Owns its own vite + Chrome via the
...
export LAB_CDP_PORT="${LAB_CDP_PORT:-9293}"
. "$(dirname "$0")/lab-servers.sh"
trap lab_servers_down EXIT
lab_servers_up
node scripts/sdf-game-void-gate.mjs "$LAB_VITE_PORT" "$LAB_CDP_PORT"
