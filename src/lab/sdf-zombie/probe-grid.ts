 ...
 * setup, against the enclosure's six walls only: no bodies, no dynamic lights,
 * no field evaluations. It is the P3 "irradiance volume lookup" shape the spec
 * reserved (`docs/superpowers/specs/2026-08-24-environment-lighting-design.md`).
 *
 ...
 * enclosure is an axis-aligned box with six linear-RGB wall albedos.
 *
 * ZERO FIELD EVALUATIONS, enforced by test on the WGSL twin. Bounce is
 * analytic — a convex box cannot self-shadow a directional light, so N.L is
 * the whole visibility story. The grid is gathered on the CPU once, not
 * marched per pixel.
 */
 ...
 * per-wall overrides; the arithmetic is albedo-driven.
 */
      ...
      for (let j = 0; j < ny; j++) {
        for (let i = 0; i < nx; i++) {
          ...
          for (const dir of dirs) {
            const hit = hitEnclosure(origin, dir, box);
            ...
            samples.push({
              dir,
              radiance: wallRadiance(hit.wall, walls[hit.wall], hit.normal, light, bounce),
            });
          }
