...
// march steps. The cyclops's crater bands are the reference artefact for
// what failure looks like.
//
// HOW TRUE THAT IS, measured (2026-09-05) rather than asserted, because this
// paragraph originally cited "the relax-1.4 render gate on
// characters/strand-fixture.blob" as its proof and that file did not exist:
//
//   * Away from its seams the field is comfortably inside the bound — worst
//     |∇d| 0.61 over 20k samples at h = 1e-6.
//   * It is NOT continuous. The strand-grid seams are isolated jumps: 9
//     steps in 1.2 million along densely walked lines. A naive
//     "|∇d| ≤ 1 everywhere" test reads 24.8 on the one sample in 5000 that
//     straddles one, which is a measurement of the seam, not of the bound.
//   * Every jump measured is sub-millimetre (worst 0.441 mm) and lands where
//     the field reads ~16 mm. A strand is millimetres across, so a seam
//     cannot clear one and the tracer has an order of magnitude more room
//     than the error.
//
// The fixture now exists and render-check passes on it at relax 1.0 and 1.4,
// but note what that gate CANNOT see: it reports HOLES, and losing the
// bundle draws the parent capsule, which is MORE material. The seam sizes
// and an over-relaxed march are pinned in strand-wiring.test.ts instead.
//
