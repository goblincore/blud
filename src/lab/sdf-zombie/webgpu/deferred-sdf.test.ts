...
describe('surface-entry wgslFn parse contract', () => {
  ...
  it('the real parser sees EXACTLY marchBody’s 93 inputs, in the same order — one binding block serves both', () => {
    ...
    // 84 + 5 probe grid slots (P3 step 1) + 4 flashlight bounce spot slots (P4 step 1), positionally last.
    expect(legacy.length).toBe(93); // plus sampled-skeleton atlas and metadata textures
