...
export function fitHandCluster(members: Primitive[]): { center: Vec3; radius: number } {
  ...
  for (const m of members) {
    ...
    const s = Math.max(m.scale[0], m.scale[1], m.scale[2]) * m.radius * boxReach(m.box) * strandReach(m.strand);
...
export function createHandsGpuView(
  ...
  function apply(prims: Primitive[]) {
    ...
    for (const p of prims) {
      ...
      const r = p.radius * Math.max(p.scale[0], p.scale[1], p.scale[2]) * boxReach(p.box) * strandReach(p.strand) + 0.02;
