...
// geometries and textures are never disposed. Surface producers (the task-2
...
import { createDeferredMeshMaterial, type DeferredMeshMaterial } from './deferred-mesh';
...
export function createGameDeferredScene(scene: THREE.Scene): GameDeferredScene {
  ...
  /** (source material, receiver) -> adapter. Adapter MATERIALS are owned
   *  here; source materials/textures are never disposed by this module. */
  const adapterCache = new Map<THREE.Material, Map<ShadowReceiver, DeferredMeshMaterial>>();
  ...
  function adapterFor(src: THREE.Material, receiver: ShadowReceiver): DeferredMeshMaterial | null {
    ...
    if (verdict === 'adapt') {
      ...
      if (!perReceiver) {
        perReceiver = new Map<ShadowReceiver, DeferredMeshMaterial>();
        adapterCache.set(src, perReceiver);
      }
      let adapter = perReceiver.get(receiver);
      if (!adapter) {
        // materialEligibility vetted `src` as the Standard family.
        adapter = createDeferredMeshMaterial(src as THREE.MeshStandardMaterial, { output: 'surface', shadowReceiver: receiver });
        perReceiver.set(receiver, adapter);
      }
      return adapter;
    }
    return null;
  ...
  return {
    ...
    dispose() {
      ...
      for (const perReceiver of adapterCache.values()) {
        for (const adapter of perReceiver.values()) adapter.dispose();
      }
      adapterCache.clear();
    },
  };
