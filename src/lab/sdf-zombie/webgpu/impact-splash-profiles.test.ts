...
it('bounds runtime inputs without propagating non-finite values', () => {
  expect(resolveImpactSplashProfile({scale:NaN,speed:Infinity,opacity:2,count:999})).toEqual({scale:1,speed:1,opacity:1,count:32});
  expect(resolveImpactSplashProfile({scale:-1,speed:0,opacity:-1,count:0})).toEqual({scale:.1,speed:.1,opacity:0,count:0});
});
