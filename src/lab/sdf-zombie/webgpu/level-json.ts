...
export function parseLevelJson(raw: unknown, opts: ParseOptions = {}): LevelDef {
  ...
  list(j.spawns, 'spawns').forEach((o, i) => {
    ...
    claim(sid);
    const kind = o.kind === 'soldier' || o.kind === 'zombie' || o.kind === 'cultist' || o.kind === 'juggernaut' ? o.kind : null;
    if (!kind) errors.push(`spawn ${sid}: kind must be zombie, soldier, cultist or juggernaut`);
