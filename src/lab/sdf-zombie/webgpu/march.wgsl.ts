...
// Two more rows, used ONLY by the merged-scene path (mapScene). They are
// appended rather than folded into DATA_ROWS so the per-body texture layout —
// which march.glsl.ts mirrors and validate.ts is checked against — is
// untouched. The merged texture is both wider and taller than a per-body one.
export const ROW_BODY_SPHERE = 7;  // xyz = centre, w = radius (blend-padded)
export const ROW_BODY_RANGE = 8;   // x = clusterStart, y = clusterCount, w = maxBlendK
export const SCENE_DATA_ROWS = 9;

/** Ceiling on merged bodies. Must match MAX_BODIES in merge-bodies.ts. */
export const SCENE_MAX_BODIES = 16;
...
// Carves belonging to ONE body, addressed by its slice of the merged cluster
// list. Same structure and same fold order as applyCarves; the only difference
// is that the cluster range is passed in rather than read from counts, because
// in a merged scene `counts` describes the whole scene.
export const APPLY_CARVES_RANGE = /* wgsl */ `fn applyCarvesRange(dIn: f32, p: vec3<f32>, data: texture_2d<f32>, cStart: i32, cCount: i32, primCount: i32) -> f32 {
  var d = dIn;
  for (var c = 0; c < 8; c = c + 1) {
    if (c >= cCount) { break; }
    let ci = cStart + c;
    let range = textureLoad(data, vec2<i32>(ci, ${ROW_CLUSTER_RANGE}), 0);
    if (range.z < 0.5) { continue; }
    let start = i32(range.x);
    let count = i32(range.y);
    for (var i = 0; i < 64; i = i + 1) {
...
// The merged field: every body in one evaluation.
//
// This is the whole point of the merged march. Previously each body owned a
// proxy box and a draw, so a pixel where bodies overlap was marched once PER
// BODY — measured at 6.2x for ten bodies sharing one body's silhouette. Here a
// pixel marches once no matter how many bodies it covers.
//
// STRUCTURE, and why it is not a flat loop over all primitives:
//
//   body sphere -> cluster sphere -> primitives
//
// A ten-body scene has 60 clusters. Testing all 60 at every step of every ray
// would give back more than the merge won, so each body is rejected by ONE
// sphere test before its clusters are ever visited.
//
// FOLD ORDER. Each body is folded exactly as the per-body path folds it —
// smin across its own clusters, then its own carves — and only then combined
// with the others. Bodies combine with a HARD min, which is associative, so
// body order does not matter and, crucially, two bodies do not blend into each
// other. Folding everything into one smin chain would have merged the whole
// crowd into a single sheet of flesh at close range.
//
// sceneCfg.x = body count.
export const MAP_SCENE = /* wgsl */ `fn mapScene(p: vec3<f32>, data: texture_2d<f32>, counts: vec4<f32>, sceneCfg: vec4<f32>, noiseAmp: f32) -> f32 {
  var d = 1e9;
  let bodyCount = i32(sceneCfg.x);
  let primCount = i32(counts.x);
  for (var b = 0; b < ${SCENE_MAX_BODIES}; b = b + 1) {
    if (b >= bodyCount) { break; }
    let sphere = textureLoad(data, vec2<i32>(b, ${ROW_BODY_SPHERE}), 0);
    // The sphere radius already carries the maxBlendK * 4 padding, applied
    // when it was built — see bodySphere() in merge-bodies.ts. Culling against
    // an unpadded sphere would clip the body exactly at its blends.
    if (length(p - sphere.xyz) - sphere.w > d) { continue; }

    let brange = textureLoad(data, vec2<i32>(b, ${ROW_BODY_RANGE}), 0);
    let cStart = i32(brange.x);
    let cCount = i32(brange.y);
    ...
    for (var c = 0; c < 8; c = c + 1) {
      ...
      let range = textureLoad(data, vec2<i32>(ci, ${ROW_CLUSTER_RANGE}), 0);
      if (range.z < 0.5) { continue; }
      let bounds = textureLoad(data, vec2<i32>(ci, ${ROW_CLUSTER_BOUNDS}), 0);
      if (length(p - bounds.xyz) - bounds.w > db + brange.w * 4.0) { continue; }
      let start = i32(range.x);
      let count = i32(range.y);
      for (var i = 0; i < 64; i = i + 1) {
        if (i >= count) { break; }
        ...
        if (S.w > 0.5) { continue; }
        let k = textureLoad(data, vec2<i32>(idx, ${ROW_PRIM_B}), 0).w;
        db = smin(db, sdPrim(p, idx, data), k);
      }
    }
    db = applyCarvesRange(db, p, data, cStart, cCount, primCount);
    d = min(d, db);
  }
  if (noiseAmp <= 0.0) { return d; }
  return d + fbm(p * 3.0) * noiseAmp;
}`;

export const CALC_NORMAL_SCENE = /* wgsl */ `fn calcNormalScene(p: vec3<f32>, data: texture_2d<f32>, counts: vec4<f32>, sceneCfg: vec4<f32>, noiseAmp: f32) -> vec3<f32> {
  let e = vec2<f32>(1.0, -1.0) * 0.0015;
  return normalize(
    e.xyy * mapScene(p + e.xyy, data, counts, sceneCfg, noiseAmp) +
    e.yyx * mapScene(p + e.yyx, data, counts, sceneCfg, noiseAmp) +
    e.yxy * mapScene(p + e.yxy, data, counts, sceneCfg, noiseAmp) +
    e.xxx * mapScene(p + e.xxx, data, counts, sceneCfg, noiseAmp));
}`;
...
// wounds, no char, no AO. Those are all PER-BODY state, and carrying them
// through a merged pass means moving them into the data texture and looking
// them up by the body that was actually hit — real work, and pointless before
// the perf number justifies it. Compare against the per-body path with face,
// wounds and AO switched off, or the comparison measures the feature gap
// rather than the architecture.
//
// The tracer is identical to MARCH_BODY's, deliberately: same relaxation, same
// overshoot test, same epsilon, same hit threshold. Only the field it samples
// and the shading it skips differ.
export const MARCH_SCENE = /* wgsl */ `fn marchScene(
  worldPos: vec3<f32>,
  camPos: vec3<f32>,
  data: texture_2d<f32>,
  counts: vec4<f32>,
  ...
  let relax = relaxCfg.y > 1.0;
  var omega = select(marchCfg.y, relaxCfg.y, relax);
  var t = clamp(startT, 0.0, tMax);
  var hit = false;
  var prevRadius = 0.0;
  var stepLen = 0.0;
  ...
  let p = camPos + rd * t;
  var n = calcNormalScene(p, data, counts, sceneCfg, marchCfg.z);
  if (surfCfg2.y > 0.0) {
    n = normalize(n + vec3<f32>(
      fbm(p * 22.0), fbm(p * 22.0 + 5.0), fbm(p * 22.0 + 11.0)) * surfCfg2.y);
  }
  ...
  var scatter = vec3<f32>(0.0, 0.0, 0.0);
  if (surfCfg.w > 0.0) {
    let thin = clamp(mapScene(p + L * 0.06, data, counts, sceneCfg, 0.0) * -8.0, 0.0, 1.0);
    scatter = deepColor * thin * surfCfg.w;
  }

  let lit = albedo * (lightCfg.y + diff * lightCfg.x) * keyColor
          + keyColor * (shine * surfCfg.x + fres) * wet
          + scatter;
...
/** Helpers for the merged path, same dependency-order rule. */
export const SCENE_HELPERS = [
  SMIN, SMAX, SD_PRIM, HASH13, NOISE3, FBM,
