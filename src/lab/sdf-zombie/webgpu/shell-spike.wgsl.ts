...
export const SHELL_TRACE = /* wgsl */ `fn shellTrace(
  ...
  for (var i = 0; i < 128; i = i + 1) {
    ...
    let p = camPos + rd * t;
    let d = mapBody(p, data, counts, vec4<f32>(0.0), woundCfg, woundCfg2, vec3<f32>(0.0, 0.0, 0.0), volumeTex, volumePose0, volumePose1, volumeMin, volumeInvExtent, volumeWarp, volumeClip, vec4<f32>(0.0)).x;
    if (d < 0.0015) {
      hit = 1.0;
      if (outMode > 0.5) { return vec4<f32>(steps, steps / 16.0, 0.0, 1.0); }
      let n = calcNormal(p, data, counts, vec4<f32>(surfCfg.y, 0.0, 0.0, 0.0), woundCfg, woundCfg2, vec3<f32>(0.0, 0.0, 0.0), volumeTex, volumePose0, volumePose1, volumeMin, volumeInvExtent, volumeWarp, volumeClip, vec4<f32>(0.0));
