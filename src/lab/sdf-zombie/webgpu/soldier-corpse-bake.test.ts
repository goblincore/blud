...
function setup(parent?: THREE.Object3D, material?: THREE.Material) {
...
describe('soldier corpse bake lifecycle',()=>{
  ...
  it('routes late corpse meshes through their registered group and removes them on restore/dispose',()=>{
    const scene = new THREE.Scene();
    const group = new THREE.Group();
    scene.add(group);
    const router = createGameDeferredScene(scene);
    router.register(group, 'mesh', 'level-only');
    const bakedMaterial = createBakedChunkMaterial({ output: 'surface', shadowReceiver: 'level-only' });
    const { actor, worker, manager, damage } = setup(group, bakedMaterial.material);
    const bake = () => {
      manager.update([actor], 2);
      worker.onmessage({ data: { id: 1, result } });
      manager.update([actor], 0);
      router.sync();
      const mesh = group.children[0] as THREE.Mesh;
      expect(mesh.material).toBe(bakedMaterial.material);
      expect(router.routeOf(mesh)).toBe('mesh');
      expect(router.receiverOf(mesh)).toBe('level-only');
      return mesh;
    };
    const first = bake();
    damage(); manager.update([actor], 0); router.sync();
    expect(group.children).toHaveLength(0);
    expect(router.routeOf(first)).toBeNull();
    const second = bake();
    manager.dispose(); router.sync();
    expect(group.children).toHaveLength(0);
    expect(router.routeOf(second)).toBeNull();
    router.dispose(); bakedMaterial.dispose();
  });
